...
with tab2:
    ...
    with col1:
        st.info("🔗 Webhook: `http://localhost:5678/webhook-test/79ff58c5-a7d9-4d7a-8338-8be46dffdbba`")
    ...
    # General Webhook Executor
    st.markdown("**🔗 Execute Custom Webhook**")
    
    # Webhook URL input
    st.markdown("**Step 1: Enter Webhook URL**")
    webhook_url_input = st.text_input(
        "Webhook URL",
        placeholder="http://localhost:5678/webhook/79ff58c5-a7d9-4d7a-8338-8be46dffdbba",
        key="webhook_url_input",
        help="Copy from your n8n workflow's Webhook trigger node"
    )
    
    # Input parameters
    st.markdown("**Step 2: Enter Parameters (JSON)**")
    input_data = st.text_area(
        "Input data",
        value='{"key": "value"}',
        height=150,
        key="input_data",
        help="Optional: Send data to your workflow"
    )
    
    # Execute button
    if st.button("▶️ Execute Workflow", key="execute_btn", type="primary"):
        ...
        try:
            ...
            st.warning("Make sure n8n is running: `docker-compose up -d n8n`")
        except requests.exceptions.Timeout:
            st.error("❌ Request timeout - n8n took too long to respond")
        except Exception as e:
            st.error(f"❌ Error: {str(e)}")
